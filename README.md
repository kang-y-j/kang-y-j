...
LangGraph의 Node/State 기반 구조를 활용하여  
사용자의 이력서를 입력받고  
질문 생성 → 답변 평가 → 조건부 Reflection까지  
면접 전체 프로세스를 자동화한 에이전트입니다.

### Week 3 — 주요 경험 정리

#### 활동명 / 프로젝트명  
AI Interview Agent (LangGraph 기반)
...
## 2) Lecture Script Generator (PPT 기반 멀티모달 에이전트)

PPTX 파일에서 텍스트, 이미지, 구조 정보를 추출하고  
LLM 기반으로 강의 스크립트를 자동 생성하는 멀티모달 에이전트입니다.  
(Repository 준비 중)

---

## 3) RAG QA System (Embedding 기반 검색)

사용자 문서를 임베딩하여  
질문–문서 검색–답변 생성까지 구성한 기본 RAG 시스템입니다.  
(Repository 준비 중)
